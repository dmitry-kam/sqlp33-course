...
create foreign table inventory_store1_in (
	inventory_id serial4 NOT NULL,
	film_id int2 NOT NULL,
	store_id int2 NOT null CHECK (store_id = 1),
	last_update timestamp NOT NULL DEFAULT now()
)
inherits (inventory_store1)
server inventory_server
options (schema_name 'public', table_name 'inventory_store1');

create foreign table inventory_store2_in (
	inventory_id serial4 NOT NULL,
	film_id int2 NOT NULL,
	store_id int2 NOT null CHECK (store_id = 2),
	last_update timestamp NOT NULL DEFAULT now()
)
inherits (inventory_store2)
server inventory_server
...
begin
	if new.store_id = 1 then
		insert into inventory_store1_in values (new.*);
	elsif new.store_id = 2 then
		insert into inventory_store2_in values (new.*);
...
-- удаляем правила из 1 задачи, чтобы не мешали работе новой логики
